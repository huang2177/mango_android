...
public class MinuteHourView extends View implements OnMoveListener {
    ...
    private boolean mIsCKZS;
    public void setIsCKZS(boolean mIsCKZS){
        this.mIsCKZS = mIsCKZS;
    }
    ...
    private void setColorStyle(Context context ,boolean mYeSyle){
        ...
        this.priceLine = mYeSyle ? ContextCompat.getColor(context, R.color.color_ffffff) : ContextCompat.getColor(context, R.color.color_00A3CC);
        this.paintColor = mYeSyle ? ContextCompat.getColor(context, R.color.color_7E8595) : ContextCompat.getColor(context, R.color.color_C2EEED);
    }

    public MinuteHourView(Context context, AttributeSet attrs) {
        super(context, attrs);
...
//                false, 0, mLeftPriceTextWidth + TEXTOFFSET, true), p);
    ...
    private void drawMaxAndMinPriceRect(Canvas canvas) {
        ...
        if (mIsCKZS){
            newMaxPrice = String.valueOf(KLineUtils.getDouble(mMaxPrice));
            newMinPrice = String.valueOf(KLineUtils.getDouble(mMinPrice));
        }else {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                // TODO: 2017/8/2 zq 坐标点暂写死，正式需要接口获取
//                if (x > 580.f && x < 620.f && y > mExcludeTimeHeight - TEXTOFFSET - 20 && y < mExcludeTimeHeight - TEXTOFFSET + 20) {
//                    mIsShow = !mIsShow;
//                    KLineChartController.getInstance().getOnPointClickListener().onPointClick(mIsShow);
//                }
