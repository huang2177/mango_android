        ...
        try {
            if (measureHeight == 0) {
                ...
                for (int i = 0; i < getItemCount(); i++) {
                    ...
                    int h = child.getMeasuredHeight() + getPaddingBottom();
                    if (h > measureHeight) //采用最大的view的高度。
                        measureHeight = h;
                }
            }
            heightSpec = View.MeasureSpec.makeMeasureSpec(measureHeight, View.MeasureSpec.EXACTLY);
        } catch (Exception e) {
        }
