...
/**
 * Des: 滑动TabLayout,对于ViewPager的依赖性强
 * Created by huang on 2018/8/21 0021 17:45
 */
...
public class SlidingTabLayout extends HorizontalScrollView implements ViewPager.OnPageChangeListener, ValueAnimator.AnimatorUpdateListener {
    ...
    private void obtainAttributes(Context context, AttributeSet attrs) {
        ...
        mScrollToCurrent = ta.getBoolean(R.styleable.SlidingTabLayout_tl_scroll_to_current, false);
        ...
        mSecTabPadding = ta.getDimension(R.styleable.SlidingTabLayout_tl_tab_sec_padding, -1);
    ...
    public String getTitle(int i) {
        return mTitles != null && i >= 0 && i < mTitles.size() ? mTitles.get(i) : "";
    }
    ...
    public void addNewTab(List<String> titles) {
        if (mTitles == null) {
            mTitles = new ArrayList<>();
        }
        mTitles.clear();
            ...
            public void onClick(View v) {
                ...
                if (position != -1) {
                    if (mCurrentTab != position) {
                        ...
                        if (mListener != null) {
                            mListener.onTabSelect(position, SlidingTabLayout.this);
                        }
                    } else {
                        if (mListener != null) {
...
//            v.setPadding((int) mTabPadding, v.getPaddingTop(), (int) mTabPadding, v.getPaddingBottom());
            ...
            if (tv_tab_title != null) {
                ...
                tv_tab_title.setPadding((int) mTabPadding, dp2px(1.5f), (int) mTabPadding, dp2px(1.5f));
                if (mTextAllCaps) {
    ...
    public void setSecTabPadding(float secTabPadding) {
        mSecTabPadding = secTabPadding;
        updateTabStyles();
    }

    /**
     * 设置第二个Tab与第一个的间距 (特殊处理  必要时可以去掉)
     */

    private void dealSecPadding(int i, View v) {
        if (mSecTabPadding != -1 && i != 0) {
            LinearLayout.LayoutParams lp = (LinearLayout.LayoutParams) v.getLayoutParams();
            lp.leftMargin = (int) mSecTabPadding;
    ...
    private void scrollToCurrentTab() {
        if (mTabCount <= 0 || !mScrollToCurrent) {
            return;
        }
